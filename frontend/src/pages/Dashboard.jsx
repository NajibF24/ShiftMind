...
import { 
  Database, Zap, Users, CloudDownload, Activity, TrendingUp, Building2, 
  PenLine, RefreshCw, Globe, BarChart3, Sparkles, AlertTriangle, 
  CheckCircle, Clock, FileText, ClipboardList
} from 'lucide-react';
...
const Dashboard = () => {
  const [stats, setStats] = useState({
    total_entries: 0, ai_queries_today: 0, query_trend: 0, active_contributors: 0,
    journal_entries_today: 0, pending_workflows: 0, pending_approvals: 0, checklist_fails_7d: 0,
  ...
  const fetchHealth = async () => {
    try {
      ...
      setSystemHealth({ db: r.data.db === 'operational' ? 'Online' : 'Degraded', raw: r.data });
    } catch (e) {
      setSystemHealth({ db: 'Offline' });
    }
  };
  ...
  const kpis = [
    { icon: Database, label: 'Knowledge Entries', value: total, color: 'var(--neon-cyan)', to: '/knowledge-manager' },
    { icon: CloudDownload, label: 'OneDrive Docs', value: stats.onedrive_documents, color: 'var(--neon-green)', to: null },
    { icon: Zap, label: 'AI Queries Today', value: stats.ai_queries_today, color: 'var(--neon-blue)', 
      trend: stats.query_trend, to: '/ask' },
    { icon: Users, label: 'Active Contributors', value: stats.active_contributors, color: 'var(--neon-purple)', to: null },
    { icon: FileText, label: "Today's Journals", value: stats.journal_entries_today, color: '#3d7eff', to: '/journal' },
    { icon: Clock, label: 'Pending Workflows', value: stats.pending_workflows, color: 'var(--warning)', to: '/workflows' },
    { icon: CheckCircle, label: 'Pending Approvals', value: stats.pending_approvals, color: 'var(--warning)', to: '/approvals' },
    { icon: AlertTriangle, label: 'Checklist FAILs (7d)', value: stats.checklist_fails_7d, color: 'var(--danger)', to: '/checklists' },
  ];
  ...
  const systems = [
    ...
    { label: 'Postgres DB', status: systemHealth.db, ok: systemHealth.db === 'Online' },
    { label: 'OneDrive', status: syncStatus?.onedrive_configured ? 'Connected' : 'Awaiting', ok: syncStatus?.onedrive_configured },
    { label: 'Knowledge Sync', status: isSyncing || syncStatus?.is_running ? 'Syncing...' : (stats.last_onedrive_sync ? 'Idle' : 'Pending'), ok: !!stats.last_onedrive_sync },
  ];
    ...
    <div className="page-content page-content--wide animate-fade-in">
      <div className="section-header" style={{ marginBottom: '40px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginTop: '20px' }}>
          <button onClick={handleManualSync} disabled={isSyncing || syncStatus?.is_running} className="btn-primary" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <RefreshCw size={15} className={(isSyncing || syncStatus?.is_running) ? 'animate-spin' : ''} />
            {(isSyncing || syncStatus?.is_running) ? 'Syncing with OneDrive...' : 'Sync Data Now'}
          </button>
          
          {syncStatus?.last_run_at && (
            <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
              Last sync: {new Date(syncStatus.last_run_at).toLocaleString()}
              {syncStatus.last_result && ` (${syncStatus.last_result.files_synced || 0} files synced)`}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="stagger-children" style={{
        ...
        {kpis.map(({ icon: Icon, label, value, color, trend, to }) => {
          const CardContent = (
            <div className={`stat-card ${to ? 'hoverable-card' : ''}`} style={{ '--card-color': color, cursor: to ? 'pointer' : 'default', height: '100%' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '20px' }}>
                <div style={{
                  width: '42px', height: '42px', borderRadius: '12px', display: 'flex',
                  alignItems: 'center', justifyContent: 'center',
                  background: `color-mix(in srgb, ${color} 15%, transparent)`, border: `1px solid color-mix(in srgb, ${color} 25%, transparent)`,
                  transition: 'all 0.3s var(--ease-out-expo)',
                }} className="stat-card__icon">
                  <Icon size={20} color={color} />
                </div>
                {trend !== undefined && (
                  <span className={`badge ${trend > 0 ? 'badge--green' : (trend < 0 ? 'badge--red' : 'badge--cyan')}`} style={{ fontSize: '0.65rem' }}>
                    {trend > 0 ? '+' : ''}{trend}%
                  </span>
                )}
                {trend === undefined && <span className="badge badge--cyan" style={{ fontSize: '0.6rem' }}>LIVE</span>}
              </div>
          ...
          return to ? <Link to={to} key={label} style={{ textDecoration: 'none' }}>{CardContent}</Link> : <div key={label}>{CardContent}</div>;
        })}
      </div>
      ...
      <div className="glass-panel glass-panel--interactive" style={{ marginBottom: '32px', padding: '28px', borderLeft: '4px solid var(--neon-cyan)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
          <div>
            ...
            {marketError ? (
              <div style={{ fontSize: '0.85rem', color: 'var(--warning)', padding: '20px 0' }}>
                <AlertTriangle size={16} style={{ display: 'inline', marginBottom: '-3px', marginRight: '6px' }} />
                Unable to fetch latest news. Scraper service might be offline.
              </div>
            ) : newsData.length > 0 ? (
          ...
          <div>
            ...
            <div style={{ background: 'rgba(255,255,255,0.02)', borderRadius: '12px', border: '1px solid var(--border)', padding: '20px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
              {marketData ? (
                <>
                ...
                <div style={{ textAlign: 'center', color: 'var(--warning)', fontSize: '0.85rem', padding: '20px 0' }}>
